...
<template>
  <liv-data-table
    ...
    @detail="handleDetail"
    @edit="handleEdit"
    @delete(demoAuthority)="handleDelete"
  />
</template>
...
const handleEdit = (_data, index) => {
  ElMessage.warning(`编辑第${index + 1}行事件`)
}

const handleDelete = (data) => {
  ElMessage.error(`点击了带demoAuthority权限标识的删除按钮，删除编号：${data.no}的事件`)
}
</script>
```
:::
...
const buttons = [
  {
    ...
    onClick: (data) => {
      ElMessage.success(`点击了带demoAuthority权限标识的按钮，点赞编号：${data.no}的事件`)
    }
  },
  {
    name: '收藏',
